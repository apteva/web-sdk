...
import { AptevaClient, AptevaError, unwrapMCP } from "../src";
...
describe("app handle — HTTP routes", () => {
  ...
  test("apiKey carries through to app routes", async () => {
    ...
    const c = new AptevaClient({ baseURL: stub.url, apiKey: "sk-app" });
...
describe("app handle — MCP tools", () => {
  ...
  test("mcpURL builds the right absolute URL", () => {
    const c = new AptevaClient({ baseURL: "https://example.com" });
...
describe("sseURL", () => {
  test("appends api_key from client", () => {
    const c = new AptevaClient({ baseURL: "https://example.com", apiKey: "sk-y" });
  ...
  test("does not override explicit api_key in params", () => {
    const c = new AptevaClient({ baseURL: "https://example.com", apiKey: "sk-from-ctor" });
